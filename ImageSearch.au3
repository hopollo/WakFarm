...
If $bTesting Then
	...
	Local $result
	$result = _ImageSearch($testimage);_ImageSearch($findImage, $tolerance = 0, $transparency = 0)
	If IsArray($result) = 1 Then
		cr("! $result=" & $result[0] & "," & $result[1] & "," & $result[2] & "," & $result[3], 1)
		MouseMove($result[0]+($result[2]/2), $result[1]+($result[3]/2), 0)
...
;
;===============================================================================
Func _ImageSearch($findImage, $tolerance = 0, $transparency = 0)
	Return _ImageSearchArea($findImage, 0, 0, @DesktopWidth, @DesktopHeight, $tolerance, $transparency)
EndFunc   ;==>_ImageSearch

Func _ImageSearchArea($findImage, $x1, $y1, $right, $bottom, ByRef $x, ByRef $y, $tolerance = 0, $transparency = 0);Credits to Sven for the Transparency addition
	...
	If (UBound($array) >= 4) Then
		Local $return[4]
		$return[0] = Int(Number($array[2])); Get x, y, l, h
		$return[1] = Int(Number($array[3]))
		$return[2] = Int(Number($array[4]))
		$return[3] = Int(Number($array[5]))
		Return $return
	EndIf
...
;
;===============================================================================
Func _WaitForImageSearch($findImage, $waitSecs, $tolerance, $transparency = 0)
	$waitSecs = $waitSecs * 1000
	$startTime = TimerInit()
	While TimerDiff($startTime) < $waitSecs
		Sleep(100)
		If _ImageSearch($findImage, $tolerance, $transparency) Then
			Return True
		EndIf
	WEnd
	Return False
...
;
;===============================================================================
Func _WaitForImagesSearch($findImage, $waitSecs, $tolerance, $transparency = 0)
	$waitSecs = $waitSecs * 1000
	$startTime = TimerInit()
	While TimerDiff($startTime) < $waitSecs
		For $i = 1 To $findImage[0]
			Sleep(100)
			If _ImageSearch($findImage[$i], $tolerance, $transparency) Then
				Return $i
			EndIf
		Next
	WEnd
